...
def main():
    ...
    utilities = mytelnetlib()

    remote_conn = utilities.telnet_connect(ip_addr)
    output = utilities.login(remote_conn, username, password)

    time.sleep(1)
    remote_conn.read_very_eager()
    utilities.disable_paging(remote_conn)

    output = utilities.send_command(remote_conn, 'show ip int brief')
